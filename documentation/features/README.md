# Features

This section documents all features of Claude Observer.

- [Overview](./overview.md) - High-level feature summary
- [Status Bar Crab](./status-bar-crab.md) - Animated crab icon in the macOS menu bar
- [Session Dropdown](./session-dropdown.md) - Dropdown menu listing all active sessions
- [Notifications](./notifications.md) - Desktop notifications and sound alerts
- [Session Lifecycle](./session-lifecycle.md) - Hook events and session state management
- [Crab Names](./crab-names.md) - Deterministic crab name assignment
